...
void UART_init(UART_Uart uart, UART_Baudrate baudrate)
{
    if (uart == UART1)
    {
        PIN_PinConfig pinConfig1 = {PORTC, PIN4, DIGITAL, PERIPHERAL};
        PIN_initPin(&pinConfig1);
        PIN_PinConfig pinConfig2 = {PORTC, PIN5, DIGITAL, PERIPHERAL};
        PIN_initPin(&pinConfig2);
        PIN_setPCTL(PORTC, PIN4, UART_F);
        PIN_setPCTL(PORTC, PIN5, UART_F);
...
{
    while ((Memory(uart, UART_FLAG) & 0x0010) != 0)
        ; // wait until RXFE is 0 (reciver is empty)
    return (uint8_t)Memory(uart, UART_DATA);
}

void UART_sendChar(Uart uart, const uint8_t data)
{
    while ((Memory(uart, UART_FLAG) & 0x0020) != 0)
        ; // wait until TXFF is 0 (transmitter is  full)
    Memory(uart, UART_DATA) = data;
}

void UART_sendString(Uart uart, const uint8_t *data)
{
    uint8_t j = 0;
    while (data[j] != '\0')
    {
...
void UART_attachBuffer(Uart uart)
{
    ...
    if (uart == UART1)
        NVIC_EN0 |= (1 << 6); //enable interrupts for UART1
